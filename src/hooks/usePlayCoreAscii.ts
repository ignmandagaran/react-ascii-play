...
export function usePlayCoreAscii(
  element: HTMLPreElement | HTMLCanvasElement | null,
  ...
  const mergedSettings = useMemo(
    () => ({ ...defaultSettings, ...program.settings, element }),
    [element, program.settings]
  );

   // Get current context
   const getContext = useCallback((): PlaygroundContext | null => {
    if (!element || !metricsRef.current) return null;
  ...
  const handlePointerMove = useCallback(
    ...
    [program, getContext, element, getCursor]
  );
  ...
  useEffect(() => {
    ...
    element.style.backgroundColor = mergedSettings.backgroundColor || '';
    element.style.color = mergedSettings.color || '';
    element.style.fontFamily = mergedSettings.fontFamily || '';
    element.style.fontSize = mergedSettings.fontSize || '';
    element.style.fontWeight = mergedSettings.fontWeight || '';
    element.style.letterSpacing = mergedSettings.letterSpacing || '';
    element.style.lineHeight = mergedSettings.lineHeight || '';
    element.style.textAlign = mergedSettings.textAlign || '';
    ...
    metricsRef.current = calcMetrics(element);
